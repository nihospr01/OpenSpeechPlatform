...
std::string OspProcess::set_params(PGroupType group, int channels) {
    ...
    for (int ch = 0; ch < 2; ch++) {
        ...
        case kBf:
            ...
            bf->set_bf_params(params::bf, params::bf_nc_on_off, params::bf_imu_on_off, params::bf_amc_on_off,
                              params::nc_thr, params::amc_thr, params::amc_forgetting_factor);
            break;
        ...
        case kWdrcCal: {
            for (int band = 0; band < params::num_bands; band++) {
                wdrcs[ch][band]->set_calibration(params::cal_mic[ch][band], params::cal_rec[ch][band],
        ...
        case kRec: {
            if (params::audio_rfile.length() == 0) {
                std::cout << "Error: empty record filename" << std::endl;
                break;
            }
            frecord.set_params(params::audio_rfile, params::audio_record);
        } break;
        default:
            break;
        }
...
void OspProcess::get_params() {
    ...
    bf->get_bf_params(params::bf, params::bf_nc_on_off, params::bf_imu_on_off, params::bf_amc_on_off, params::nc_thr,
    ...
    for (int ch = 0; ch < 2; ch++) {
        ...
        for (int band = 0; band < params::num_bands; band++) {
            ...
            wdrcs[ch][band]->get_param(params::g50[ch][band], params::g80[ch][band], params::knee_low[ch][band],
                                       params::mpo_band[ch][band]);
            wdrcs[ch][band]->get_calibration(params::cal_mic[ch][band], params::cal_rec[ch][band],
                                             params::cal_mpo[ch][band]);
        }
    }
}

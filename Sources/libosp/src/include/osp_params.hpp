...
enum {
    ...
    kForce = 8 // Force setting, even if cached value shows no change.
};

typedef enum { kCommon, kBf, kFp, kPlay, kSpeech, kWdrc, kWdrcCal, kAfc, kRec } PGroupType;
...
std::vector<float> cal_mic[2];
std::vector<float> cal_rec[2];
std::vector<float> cal_mpo[2];
float global_mpo[2];
int aligned[2];
int min_phase[2];
...
ParamValue par[] = {
    ...
    {.name = "bf_imu_on_off", .type = kInt, .flags = kLeft, .group = kBf, .param = &bf_imu_on_off},
    ...
    // audio play
    {.name = "audio_filename", .type = kStr, .flags = kLeft, .group = kPlay, .param = &audio_filename},
    {.name = "audio_play", .type = kInt, .flags = kForce | kCmd | kLeft, .group = kPlay, .param = &audio_play},
    {.name = "audio_repeat", .type = kInt, .flags = kLeft, .group = kPlay, .param = &audio_repeat},
    {.name = "audio_playing", .type = kInt, .flags = kLeft, .group = kPlay, .param = &audio_playing},
    {.name = "audio_reset", .type = kInt, .flags = kForce | kCmd | kLeft, .group = kPlay, .param = &audio_reset},
    {.name = "audio_alpha", .type = kFloat, .flags = kLeft, .group = kPlay, .param = &audio_alpha},
    // audio record
    {.name = "audio_rfile", .type = kStr, .flags = kLeft, .group = kRec, .param = &audio_rfile},
    {.name = "audio_record", .type = kInt, .flags = kForce | kLeft, .group = kRec, .param = &audio_record},
    ...
    {.name = "min_phase", .type = kInt, .flags = kBoth, .group = kWdrc, .param = &min_phase},
    {.name = "aligned", .type = kInt, .flags = kBoth, .group = kWdrc, .param = &aligned},
    // WDRC (calibration)
    {.name = "cal_mic", .type = kArray, .flags = kBoth, .group = kWdrcCal, .param = &cal_mic},
    {.name = "cal_rec", .type = kArray, .flags = kBoth, .group = kWdrcCal, .param = &cal_rec},
    {.name = "cal_mpo", .type = kArray, .flags = kBoth, .group = kWdrcCal, .param = &cal_mpo},
    // AFC
    ...
    {.name = "afc_reset", .type = kFloat, .flags = kForce | kCmd | kBoth, .group = kAfc, .param = &afc_reset},
};

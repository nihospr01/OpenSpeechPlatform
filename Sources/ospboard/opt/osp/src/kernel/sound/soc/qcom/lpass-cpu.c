		...
		struct snd_soc_dai *dai)
{
	...
	/* hack: disable TER bit_clk as we are using external bit clk */
	if (dai->driver->id == MI2S_TERTIARY)
		return 0;
		...
		struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	reg_wssrc = LPAIF_I2SCTL_WSSRC_INTERNAL;
#ifdef CONFIG_SND_SOC_APQ8016_OSP
	/*
	 * OSPMOD - SKH and Louis
	 * Within the MI2S peripherals themselves (not the routing to them),
	 * Primary and Quaternary have to be set to use external WS
	 * (and possibly Tertiary depending on the configuration)
	 */
	if (dai->driver->id == MI2S_PRIMARY
	    || dai->driver->id == MI2S_TERTIARY
	    || dai->driver->id == MI2S_QUATERNARY)
		reg_wssrc = LPAIF_I2SCTL_WSSRC_EXTERNAL;
#endif
	...
	if(drvdata->mi2s_bit_clk[id])
	    printk(KERN_WARNING "MI2S_%dARY: setting bit clock %s to %d Hz (%d bit %d Hz spl)\n",
	            (id + 1), __clk_get_name(drvdata->mi2s_bit_clk[id]),
	            rate * bitwidth * 2, bitwidth, rate);
	else
	    printk(KERN_WARNING "MI2S_%dARY has no bit clock\n", (id + 1));

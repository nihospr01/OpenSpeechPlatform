...
class BeamformingDemo extends Component {
    ...
    loadParameter = async () => {
        try {
            ...
            this.setState({
                ...
                imuGesturesEnabled: data['left']['bf_imu_on_off'] == 1
            });
        }
        catch (error) {
            throw error;
    ...
    setMu = async (value) => {
        ...
        await this.handleTransmiteBeamformingData(beamformingEnabled, value, ncEnabled, amcEnabled, imuGesturesEnabled);
    ...
    handleNcChange = async () => {
        ...
        try {
            await this.handleTransmiteBeamformingData(beamformingEnabled, mu, !ncEnabled, amcEnabled, imuGesturesEnabled);
            this.setState({
                ncEnabled: !ncEnabled,
            });
        }
    ...
    handleGestureToggle = async () => {
        ...
        try {
            await this.handleTransmiteBeamformingData(beamformingEnabled, mu, ncEnabled, amcEnabled, !imuGesturesEnabled);
            this.setState({
                imuGesturesEnabled: !imuGesturesEnabled,
            });
        }
        catch(error) {
            alert(error.response.data);
    ...
    handleTransmiteBeamformingData  = async (beamformingEnabled, mu, ncEnabled, amcEnabled, imuGesturesEnabled ) => {
        let beamformingData = {
            'bf':beamformingEnabled === false ? 0 : 1,
            'bf_mu': mu,
            'bf_nc_on_off': ncEnabled === false ? 0 : 1,
            'bf_amc_on_off': amcEnabled === false ? 0 : 1,
            'bf_imu_on_off': imuGesturesEnabled === false ? 0 : 1
        }
            ...
            <div className={classes.root}>
                <Grid container direction="column" justify="center">
                    <div>
                        ...
                        <Grid item>
                            <div>
                                Enable beamforming using IMU gestures
                                <Switch
                                    checked={this.state.imuGesturesEnabled}
                                    onChange={this.handleGestureToggle}
                                    color="primary"
                                />
                            </div>
                        </Grid>
                    ...
                    <Grid container alignItems="center">
                        <Button 
			    style={{maxWidth: '300px', maxHeight: '200px', minWidth: '30px', minHeight: '30px'}}
                            variant="contained"
                            color={beamformingEnabled ? "primary" : "secondary"}

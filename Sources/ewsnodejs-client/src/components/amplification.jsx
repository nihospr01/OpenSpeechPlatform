...
class Amplification extends Component {
    ...
    updateParamTable = (data) => {
        ...
        // Note: arrays are reference types in js. g65_Left would be reference to param_left["g50"].
        //       That is why when g65_Left is modified param_left["g50"] is also modified. 
        // var g65_Left = param_left["g50"]; 
        // var g65_Right = param_right["g50"]; 

        const bandNumber = g50_Left.length;

        // the values will be modified below. 
        var g65_Left = Array(bandNumber).fill(1);
        var g65_Right = Array(bandNumber).fill(1);
        var cr_Left = Array(bandNumber).fill(1);
        var cr_Right = Array(bandNumber).fill(1);

        for (var i = 0; i < bandNumber; i++) {
            // compute g65 based on g80 & g50
            var slope_L = (g80_Left[i] - g50_Left[i]) / 30;
            // typo fixed
            var slope_R = (g80_Right[i] - g50_Right[i]) / 30;
            var leftVal = Number(g50_Left[i]) + Number(slope_L * 15);
            var rightVal = Number(g50_Right[i]) + Number(slope_R * 15);
            g65_Left[i] = leftVal;
            g65_Right[i] = rightVal;

            // compute cr based on g80 & g50
            cr_Left[i] = (80-50) / ((80+g80_Left[i])-(50+g50_Left[i]));
            cr_Right[i] = (80-50) / ((80+g80_Right[i])-(50+g50_Right[i]));
        }
        ...
        let stateData = {
            ...
            // previously cr is switched to 1. But we want to display the correct value of cr. 
            // the correct value should be calculated by the formula. 
            ...
            <div className={classes.content}>
                ...
                <Grid container spacing={3}>
                    <Grid item md={6} xs={12} >
                        <Paper style={{ maxWidth: 200, marginTop: 30 }}>
                        <List
                            subheader={
                                <ListSubheader>Band Number: {this.state.bandNumber}</ListSubheader>
                            }
                            ></List>

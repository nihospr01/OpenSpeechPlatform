...
class AudioFilePlayer extends Component {
    ...
    componentDidMount = async () => {
        try {
            ...
            // const fileResponse = await axios.get("/api/researcher/fileNames?folder_name=");
    ...
    /* Receive parameters from MHA, update state */
    updateParam = async () => {
        try {
            ...
            // when audio stops playing, stop receving parameters from MHA. 
            var {id} = this.state;
            if (data.left.audio_playing == 0){
                clearInterval(id);
            } 
            this.setState({id});
        } catch (error) {
    ...
    playFile = async () => {
        ...
        // when audio starts playing, start calling this.updateParam. 
        var {id} = this.state;
        id = setInterval(this.updateParam, 500); // the frequency can be tuned, e.g, 1000, 2000, 3000
        this.setState({id});
        ...
        const audioData = {
            ...
            audio_filename: "audio/" + currentFile,
            // alpha: 1,
            audio_alpha: 1,
            audio_play: 1
        };
    ...
    render() {
        ...
        const { audio_play, audio_repeat } = this.state;

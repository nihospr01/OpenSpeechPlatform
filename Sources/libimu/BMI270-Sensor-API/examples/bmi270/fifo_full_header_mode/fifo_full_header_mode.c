    ...
    while (try <= 10)
    {
        ...
        if ((rslt == BMI2_OK) && (int_status & BMI2_FFULL_INT_STATUS_MASK))
        {
            ...
            if (rslt == BMI2_OK)
            {
                ...
                printf("frame_count: %d  accel_count: %d  gyro_count: %d\n", frame_count, accel_frame_length, gyro_frame_length);
                ...
                printf("Sensor time = %u\r\n", fifoframe.sensor_time);
                float ttime = (float)diff(time1,time3).tv_nsec / 1e6;
                printf("TIMES: %f.2 ms  |  %f.2 ms\n", (float)diff(time1,time2).tv_nsec / 1e6, (float)diff(time2,time3).tv_nsec / 1e6);
                printf("RATE: %f.2 Hz\n", 1000/ttime * frame_count);
